import { type Component, createMemo, type ParentComponent, type JSXElement, createSignal, Show, onMount } from "solid-js";
import BlogTopGallery from "./blog-top-gallery";

const P: ParentComponent = (props) => <p class="mt-1v">{props.children}</p>;

const Ol: ParentComponent = (props) => <ol class="list-decimal [&>li]:ml-3h">{props.children}</ol>;
const Ul: ParentComponent = (props) => <ul class="list-square [&>li]:ml-2h">{props.children}</ul>;

const Li: ParentComponent = (props) => <li class="">{props.children}</li>;

export const Blockquote: ParentComponent = (props) => <blockquote>{props.children}</blockquote>;

const Pre: ParentComponent<{ lang: string; lines?: string; file?: string }> = (props) => {
...
const headingLink = (children: JSXElement) => children?.toString().toLowerCase().replaceAll(" ", "-").replaceAll(",", "");

const HeadlineLink: Component<{ link: string; class: string }> = (props) => {
  return (
    <a href={props.link} class="relative top-[1px]">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class={props.class} fill="none">
        <title>link</title>
        <path d="M9.52051 14.4359L14.4335 9.52283" stroke="currentColor" stroke-width="3" stroke-linecap="round" />
        <path
...
const H2: ParentComponent = (props) => (
  <h2 id={headingLink(props.children)} class="text-2xl leading-2 font-bold mt-2v mb-1v flex items-center gap-1h scroll-mt-2v">
    {props.children}
...
const H3: ParentComponent = (props) => (
  <h3 id={headingLink(props.children)} class="text-xl leading-2 font-bold mt-2v mb-1v flex items-center gap-1h scroll-mt-2v">
    {props.children}
...
const H4: ParentComponent = (props) => (
  <h4 id={headingLink(props.children)} class="text-lg leading-1 font-bold mt-2v mb-1v flex items-center gap-1h scroll-mt-2v">
    {props.children}
...
const A: ParentComponent<{ href: string }> = (props) => {
  const isLocal = createMemo(() => ["/", "./", "#"].some((s) => props.href.startsWith(s)));

  return (
    <a href={props.href} target={isLocal() ? "" : "_blank"} class="underline underline-offset-2">
      {props.children}
    </a>
  );
};
...
export const PostImage: Component<{
  ...
  return (
    <div>
      <img ref={ref} src={props.src} alt={props.alt} class="w-full" classList={{ [props.class || ""]: !!props.class }} />
      {props.attr}
    </div>
  );
};
